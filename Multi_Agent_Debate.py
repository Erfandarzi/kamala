...
def modelslab_api_call(prompt, system_message):
    url = "https://modelslab.com/api/v6/llm/uncensored_chat"
    payload = json.dumps({
        "key": st.secrets["MODELSLAB_API_KEY"],  # Store your API key in Streamlit secrets
        "messages": [
            {
                "role": "system",
...
def generate_agent_description(name: str, conversation_description: str, word_limit: int) -> str:
    ...
    agent_specifier_prompt = [
        ...
        HumanMessage(
            content=(
                ...
                f"in {word_limit} words or less in English.\n"
    ...
    agent_specifier_llm = ChatOpenAI(model=st.session_state.model, temperature=1.0)
...
def set_debate() -> None:
    ...
    # Remove the language selection entirely
    st.session_state.language = "English"  # Set default language to English

    st.write("**Model**")
    st.session_state.model = st.radio(
        label="Model",
        options=("gpt-4o-mini", "gpt-4o", "modelslab"),
        label_visibility="collapsed",
        horizontal=True,
        index=1,
    )
...
def multi_agent_debate() -> None:
    ...
    with st.sidebar:
        st.image("files/logo.svg", width=100)  # Adjust the width as needed
        st.write("")
    ...
    if authentication:
        if not st.session_state.ready:
            if is_openai_api_key_valid(openai_api_key):
                ...
                st.info(
                    """
                    This app is coded by Sundai Club
                    """
                )
                st.stop()
    else:
    ...
    with st.sidebar:
        st.write("---")
        st.image("files/logo.svg", width=60)  # Adjust the width as needed
